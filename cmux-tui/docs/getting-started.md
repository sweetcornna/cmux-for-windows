...
Windows builds target `x86_64-pc-windows-gnu` and need mingw-w64 GCC and libclang as well; see [Windows](windows.md).
...
cmux-tui supports macOS, Linux, and Windows. Windows runs terminals on ConPTY and is a local multiplexer only: remote daemon and machine commands are compiled out there, and [Windows](windows.md) has the full support matrix. The TUI config path resolves `CMUX_TUI_CONFIG`, then legacy `CMUX_MUX_CONFIG`, then `$XDG_CONFIG_HOME/cmux/cmux-tui.json` or `~/.config/cmux/cmux-tui.json`; on Windows it resolves `%APPDATA%\cmux\cmux-tui.json`. Existing `mux.json` files remain supported and are used when `cmux-tui.json` is absent.
