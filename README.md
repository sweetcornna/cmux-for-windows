...
> **This fork adds a native Windows GUI and Windows support for `cmux-tui`.**
> Download the latest per-user `Setup.exe` from the
> [Windows releases](https://github.com/sweetcornna/cmux-for-windows/releases/latest),
> or use the standalone TUI. Terminals run on ConPTY and the public resource CLI
> works unchanged; the remote daemon and machine commands remain Unix-only. The
> macOS app itself is unchanged and still macOS-only. See
> [cmux-tui/docs/windows.md](cmux-tui/docs/windows.md) for the support matrix,
> installer behavior, and build instructions. Translated READMEs below describe
> upstream cmux and do not yet mention this.
