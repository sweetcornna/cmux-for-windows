...
8. Put terminals in different local directories, start Claude Code, OpenCode, and Codex sessions, then restart the main application. Confirm workspace, screen, pane, tab, layout, focus, names, browser URLs, per-terminal directories, and provider sessions are restored with new ConPTY processes and fresh WebView controls. Terminal output, scrollback, arbitrary commands, page state, and browser history must not be restored. Confirm working/needs-attention/done status appears, completion and attention produce a notification only inside the cmux window, no Windows system notification is sent, and the in-app focus action selects the exact workspace, screen, pane, and tab.
